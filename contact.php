...
?>
    ...
    <form action="#" method="post">
       <div>
           <input type="text" id="contactName" name="contactName" placeholder="Your Name" novalidate>
       </div>
       <div>
           <input type="email" id="contactEmail" name="contactEmail" placeholder="Your Email" novalidate>
       </div>

       <div>
           <input type="text" id="contactPhone" name="contactPhone" placeholder="Your Number" novalidate>
       </div>

       <div>
           <input type="text" id="contactSubject" name="contactSubject" placeholder="Subject" novalidate>
       </div>

       <div>
           <textarea id="contactMessage" name="contactMessage" rows="4" cols="30" placeholder="Your Message" novalidate></textarea>
       </div>
       ...
       document.addEventListener("DOMContentLoaded", function () {
           ...
           form.addEventListener("submit", function (event) {
               ...
               if (contactNameInput.value.trim() === "") {
                   isValid = false;
                   displayValidationMessage(contactNameInput, "Name is required.");
               }

               // Validate Email
               if (contactEmailInput.value.trim() === "") {
                   isValid = false;
                   displayValidationMessage(contactEmailInput, "Email is required.");
               } else if (!emailPattern.test(contactEmailInput.value.trim())) {
                   isValid = false;
                   displayValidationMessage(contactEmailInput, "Invalid email format.");
               }

               // Validate Phone Number
               if (contactPhoneInput.value.trim() === "") {
                   isValid = false;
                   displayValidationMessage(contactPhoneInput, "Phone number is required.");
               } else if (!phonePattern.test(contactPhoneInput.value.trim())) {
                   isValid = false;
                   displayValidationMessage(contactPhoneInput, "Phone number must be 10 digits.");
               }

               // Validate Subject
               if (contactSubjectInput.value.trim() === "") {
                   isValid = false;
                   displayValidationMessage(contactSubjectInput, "Subject is required.");
               }

               // Validate Message
               if (contactMessageInput.value.trim() === "") {
                   isValid = false;
                   displayValidationMessage(contactMessageInput, "Message is required.");
               }
